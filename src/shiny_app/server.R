...
tissue_annotation <-
  HeatmapAnnotation(tissue = sample_collection_metadata$tissue,
                    col = list(
                      tissue = c("cerebellum" = "#F8766D", "cortex" = "#7CAE00",
                                 "hippocampus" = "#00BFC4", "striatum" = "#C77CFF")))
...
server <- function(input, output, session) {
  ...
  output$heatmap <- renderPlot({
    ...
    Heatmap(as.matrix(combined_cpm[input$gene_ids, ]), name = "cpm",
            ...
            show_column_names = FALSE, col = ocean.deep(10))
  })
  ...
  updateSelectizeInput(session, "gene_name3", choices = region_region_switchlist_analyzed$isoformFeatures$gene_name,
                       server = TRUE)
  # fetch data
  data_3 <- reactive(region_sex_switchlist_list_analyzed[[input$brain_region_3]]$isoformFeatures)
