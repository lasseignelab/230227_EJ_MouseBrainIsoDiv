...
The goal of this script is to compare my DTU genes to known alzheimers disease, psychiatric disorder risk, and cpam case genes. It is dependent on scripts 01 - 07.

I am going to split this analysis by brain region.
...
Now, I can use the results from this object to get my significant gene lists, Although they should all be saved already as outputs. To keep the ensembl ids, I will be doing this.
...
region_region_genes <- unique(
  region_region_switchlist_analyzed[["isoformFeatures"]]$short_id)
...
Next, I need to compare this cleaned up list with the intersect function to my DTU genes in each tissue.

```{r compare intersections}
...
Next, I check for overlap of my genes with mouse psychiatric genes.

```{r check for psych overlap}
...
and now compare overlap with my gene lists

```{r compare cpam overlap}
