...
# but I choose to do the others as well just so DESeq2 results can be saved.
add_save_orfs_sex("cerebellum")
add_save_orfs_sex("cortex")
add_save_orfs_sex("hippocampus")
add_save_orfs_sex("striatum")
```
Now, we can save the isoform switching events plots.

```{r plot isoform switch events for striatum}
get_gene_symbols("striatum_sex") # need to do this to get gene symbol for title
...
fptm[3] / 60
```
This script takes 36 minutes to run, mostly during add_save_orfs.
