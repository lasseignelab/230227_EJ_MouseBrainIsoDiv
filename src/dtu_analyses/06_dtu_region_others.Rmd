...
# run function for making switchlist on all brain regions
make_switchlist_run_saturn(
  tissue = "cerebellum",
...
# check switch summary for each analysis
extractSwitchSummary(cerebellum_switchlist_analyzed)
extractSwitchSummary(cortex_switchlist_analyzed)
extractSwitchSummary(hippocampus_switchlist_analyzed)
extractSwitchSummary(striatum_switchlist_analyzed)
```

Expected results:
cerebellum vs other	186	136	129
cortex vs other	29	15	21	
hippocampus vs other	64	61	48	
other vs striatum	3	2	2	
...
get_sig_genes(
  tissue_switchlist = cerebellum_switchlist_analyzed,
  save_path = here("results", "dtu_genes")
)
get_sig_genes(
  tissue_switchlist = cortex_switchlist_analyzed,
  save_path = here("results", "dtu_genes")
)
get_sig_genes(
  tissue_switchlist = hippocampus_switchlist_analyzed,
  save_path = here("results", "dtu_genes")
)
get_sig_genes(
  tissue_switchlist = striatum_switchlist_analyzed,
  save_path = here("results", "dtu_genes")
)

# make all into list
all_gene_list <- list(
  cerebellum = na.omit(cere_sig_genes),
  cortex = na.omit(cort_sig_genes),
  hippocampus = na.omit(hipp_sig_genes),
  striatum = na.omit(stri_sig_genes)
)
```
...
# cortex isoforms
cort_sig_isoforms <- unique(cort_sig_features$isoform_id)

# hippocampus isoforms
hipp_sig_isoforms <- unique(hipp_sig_features$isoform_id)

# striatum isoforms
stri_sig_isoforms <- unique(stri_sig_features$isoform_id)

# make all into list
all_isoform_list <- list(
  cerebellum = na.omit(cere_sig_isoforms),
  cortex = na.omit(cort_sig_isoforms),
  hippocampus = na.omit(hipp_sig_isoforms),
  striatum = na.omit(stri_sig_isoforms)
)
```
...
# plot
create_volcano_plot_region(
  tissue_obj = cerebellum_switchlist_analyzed,
  save_path = here(
    "results",
    "plots", "satuRn_volcano"
  )
)

create_volcano_plot_region(
  tissue_obj = cortex_switchlist_analyzed,
  save_path = here(
    "results",
    "plots", "satuRn_volcano"
  )
...
create_volcano_plot_region(
  tissue_obj = hippocampus_switchlist_analyzed,
  save_path = here(
    "results",
    "plots", "satuRn_volcano"
  )
...
create_volcano_plot_region(
  tissue_obj = striatum_switchlist_analyzed,
  save_path = here(
    "results",
    "plots", "satuRn_volcano"
  )
