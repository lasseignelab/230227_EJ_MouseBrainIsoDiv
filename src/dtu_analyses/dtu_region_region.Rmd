...
# this results in 44024 isoforms left
cpm_switchlist <- preFilter(cpm_switchlist, geneExpressionCutoff = NULL)

# now run DEXSeq
cpm_switchlist_analyzed <- isoformSwitchTestDEXSeq(
  switchAnalyzeRlist = cpm_switchlist,
  reduceToSwitchingGenes = TRUE,
  reduceFurtherToGenesWithConsequencePotential = FALSE
)

extractSwitchSummary(cpm_switchlist_analyzed)

# save object for further analyses
saveRDS(cpm_switchlist_analyzed, here(
  "data", "switchlist_objects",
  "region_region_switchlist_analyzed.Rds"
))

# get gene symbols
get_gene_symbols("cpm")

# save object with gene symbols for further analyses
saveRDS(cpm_switchlist_analyzed, here(
  "data", "switchlist_objects",
  "region_region_switchlist_analyzed.Rds"
))
...
cerebellum vs cortex	1	1	1	
cerebellum vs hippocampus	13	10	9	
cerebellum vs striatum	2	1	1	
cortex vs hippocampus	59	46	39	
cortex vs striatum	0	0	0	
hippocampus vs striatum	39	35	28	
Combined	98	90	66	
...
sig_isoform_features <- cpm_switchlist_analyzed$isoformFeatures
...
```{r volcano plot}
# rename so function gets expected input. go back and change if needed.
cpm_switchlist_dexseq <- cpm_switchlist_analyzed
...
run_plot_gprofiler(sig_isoform_genes_cere_stri) # skip
run_plot_gprofiler(sig_isoform_genes_cort_hipp)
run_plot_gprofiler(sig_isoform_genes_cort_stri) # skip
run_plot_gprofiler(sig_isoform_genes_hipp_stri) # skip
```
...
I was able to run everything in 50 minutes.
