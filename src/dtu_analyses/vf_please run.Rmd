...
saveRDS(region_region_switchlist, "/data/project/lasseigne_lab/VictoriaFlanary/EJ_code_review/region_region_switchlist_ej.Rds")
...
saveRDS(switchlist_analyzed_true, "/data/project/lasseigne_lab/VictoriaFlanary/EJ_code_review/switchlist_analyzed_true_ej.Rds")
...
## rerun vf
switchlist_analyzed_false_vf <- isoformSwitchTestSatuRn(
  switchAnalyzeRlist = region_region_switchlist_vf,
  reduceToSwitchingGenes = FALSE
)

extractSwitchSummary(switchlist_analyzed_false_vf)
...
```{r round the sva after 10 decimal points}

region_region_switchlist_analyzed[["designMatrix"]][,3:12] <- round(region_region_switchlist_analyzed[["designMatrix"]][,3:12], digits = 11)

region_region_switchlist_vf[["designMatrix"]][,3:12] <- round(region_region_switchlist_vf[["designMatrix"]][,3:12], digits = 11)

waldo::compare(region_region_switchlist_analyzed[["designMatrix"]], region_region_switchlist_vf[["designMatrix"]])

diffs <- waldo::compare(region_region_switchlist_analyzed[["isoformRepExpression"]], region_region_switchlist_vf[["isoformRepExpression"]])
```