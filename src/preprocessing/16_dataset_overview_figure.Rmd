...
annotation <- read_table(
  ...
  col_names = c("location", "method", "type", "start", "stop", "blank1", "strand", "blank2", "geneid", "gene_id", "transcriptid", "transcript_id")
)
...
# save
ggsave(reads_per_tissue_plot, here("results", "plots", "bar_plots", "reads_per_sample.png"),
  width = 6, height = 5
)
...
# barplot
novel_genes_plot <- ggplot(gene_data, aes(x = gene_novelty, y = count, fill = gene_novelty)) +
...
# save
ggsave(novel_genes_plot, here("results", "plots", "bar_plots", "novel_genes_barplot.png"),
  width = 3, height = 4
)
```
...
# save
ggsave(novel_txs_plot <- here("results", "plots", "bar_plots", "novel_transcripts_barplot.png"),
  width = 3, height = 4
)
```
...
# save
ggsave(tx_per_gene_plot, here("results", "plots", "histograms", "transcripts_per_gene.png"))
...
novel_transcripts_annotation <- transcripts_annotation[transcripts_annotation$transcript_id %in% novel_txs$TXNAME,]
...
novel_genes_annotation <- transcripts_annotation[transcripts_annotation$gene_id %in% novel_genes,]
...
top_row <- plot_grid(vst_tissue_pca, reads_per_tissue_plot, labels = c("B", "C"), rel_widths = c(2, 1))

top_row

bottom_row <- plot_grid(novel_genes_plot, novel_txs_plot, tx_per_gene_plot, labels = c("D", "E", "F"), rel_widths = c(0.5, 0.5, 1), nrow = 1)
...
png(here("results", "finished_figures", "figure1.png"), width = 8, height = 7, units = "in", res = 300)
final_figure1
dev.off()
...
rank_sum <- wilcox.test(merged_cpm_iso_mean_df$means[merged_cpm_iso_mean_df$novel == TRUE],
...
novel_cpm_means_df <- merged_cpm_iso_mean_df[merged_cpm_iso_mean_df$novel == TRUE,]
...
novel_cpm_pivot <- pivot_longer(novel_cpm, cols = c(2:41), values_to = "CPM", names_to = "sample_id")
...
merged_cpm_iso_means_long <- pivot_longer(merged_cpm_iso_means, cols = c(2:41), values_to = "CPM", names_to = "sample_id")
