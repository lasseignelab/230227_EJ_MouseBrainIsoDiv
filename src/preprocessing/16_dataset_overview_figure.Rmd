...
- Change default transcript/gene colors so they do not look like tissue colors
- delete useless code
---
...
In this code chunk I am trying to parse out exactly the relationship between novel and annotated transcript expression values, but it is hard to see both at once.

```{r actually plot}
ggplot(merged_counts_iso_means, aes(x = means)) +
  geom_histogram(
    data = subset(merged_counts_iso_means, novel == TRUE),
...
ggplot(merged_counts_iso_means, aes(x = log10(means))) +
  geom_density(
    data = subset(merged_counts_iso_means, novel == FALSE),
