...
```{r make dtu heatmap}
better_rownames <- c("Cerebellum vs. Striatum", "Cerebellum vs. Cortex",
...
heatmap_1 <- Heatmap(as.matrix(for_heatmap_1), name = "Jaccard Similarity", 
                     col = col_fun,
                     row_labels = better_rownames,
                     row_order = c(3,1,2,6,4,5),
                     row_names_gp = gpar(fontface = "bold"),
                     column_labels = c("Cerebellum DTU", "Cortex DTU",
...
for_heatmap_2 <- as.data.frame(pivot_wider(all_dge_combinations, names_from = `names(aggregate_dge_comparisons)`, values_from = jaccard_similarity))
...
heatmap_2 <- Heatmap(as.matrix(for_heatmap_2), 
                     ...
                     row_labels = better_rownames,
                     row_names_gp = gpar(fontface = "bold"),
                     column_labels = c("Cerebellum DGE", "Striatum DGE",
...
switchPlot(region_region_switchlist, gene = cerebellum_present_list[1], condition1 = "cerebellum", condition2 = "striatum")

switchPlot(region_all_switchlist_list[["cerebellum"]], gene = cerebellum_not_present_list[1])

switchPlot(region_region_switchlist, gene = cerebellum_not_present_list[1], condition1 = "cerebellum", condition2 = "striatum")
```
